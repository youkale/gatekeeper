...
| Command | Purpose |
| --- | --- |
| `gatekeeper dispatch start [--issue <n>] [--brief <file>] [--agent-command <cmd>] [--run-timeout <s>] [--repo <org/name>] [--registry <dir>] [--yes]` | Create a work order and run the front-of-terminal supervision loop until a terminal/report state. At least one of `--issue`/`--brief` is required: `--issue` alone dispatches that GitHub issue (brief synthesized from its body plus the target repo's triage ledger — last matching entry wins); `--brief` alone starts an **ad-hoc order with no GitHub issue at all** (no GitHub API call, association key `org/repo@adhoc-<id>`); both together use `--brief` as the task package verbatim, with `--issue` only supplying the `org/repo#N` association key. |
