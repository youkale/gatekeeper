...
- [T-20260720-09 + T-20260721-09] 退出码表同类翻车 ×3（DISPATCH 表 resume/cancel 合并句、修复时 start 误归类、REVIEW 表 arbitrate 分配句式）→ 规范：**退出码/行为枚举表一命令一从句，禁止跨命令共享形容词/状态限定**；文档任务包固定条款 → 处置：REVIEW.md §1.3 已含警示段，本条入文档类任务包模板。
- [T-20260721-09] 文档精确化的反向审计价值实证：G 包"一命令一句"重写把 E 包一个跨命令退出码不一致从模糊表述里逼出（accept/arbitrate 畸形 id 退 3 vs 其余退 2），修代码使文档成真优于改文档迁就代码 → 规范性文档写作本身是一种代码审查手段；"文档-代码矛盾"出现时优先怀疑代码而非默认改文档。

- [T-20260721-07/08 补审] 降级补审的价值第二次实证：grok 恢复后对 claude 双路（主路+同厂商替补）验收过的三包补审，独家击穿 arbitrate extend 真实路径不可运行——两路测试都 mock 掉真实 supervisor 只断言 journal 副作用，同源盲区完美互补印证 → 规范：**状态机新边/新流转的测试必须至少一条走真实驱动端到端（禁止仅 mock 断言副作用）**；"降级期结论必须补审"再次自证非形式主义 → 处置：T-20260721-10 返工；本条入编码任务包模板。

- [review dogfood 三连跑 2026-07-21] 每跑一轮剥一层真实摩擦的收敛实录：run1 契约冲突（角色卡旧格式 vs VERDICT.json）→ T-11；run2 证实契约修复生效、暴露 CLI 流式叙述运输层 → T-12；run3 证实提取生效、只读指纹首次开火但为**误归因**（活跃开发仓的并发人类写入被算给 reviewer——检测器无法区分写者），经 ARBITRATION 人工核验证据后 accept 收官（extracted verdict pass + token 全对）。两条结论：① 设计推迟项的触发条件机制有效（per-lane worktree 隔离的预设触发器精确命中）→ T-20260721-13 排队；② ARBITRATION 兜住了机器无法裁决的歧义局面——"停下、摆材料、等人裁"的价值首次实战兑现。附带：run1/2 的 file 措辞诱导 reviewer 在 cwd 写 out/VERDICT.json（污染已清、stdout 措辞已根治）。
