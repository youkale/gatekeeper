...
## 已知环境限制（规范修订 2026-07-20，同类 ×3 触发）

Codex review 沙箱为只读环境，`npm test`/vitest 必因 EPERM 失败——这是环境噪音，不是代码缺陷。归纳结论时：不得将沙箱内测试运行失败列为 finding 或验收阻断项；如 Codex 原文包含此类主张，在报告中标注「沙箱噪音，需调度者本机核实」。测试是否全绿以调度者本机运行为唯一地面事实。
