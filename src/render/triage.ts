...
function renderTierLine(selection: TierModelSelection): string {
	const status = selection.availabilityKnown
		? selection.selected.length > 0
			...
			: "no available model under current pi config"
		: "availability unknown (pi config unreadable)";
...
export function renderTriageBriefing(input: TriageBriefingInput): string {
	...
	lines.push(
		"Generated by `gatekeeper triage` -- judgement itself never happens inside this CLI (zero-model invariant). " +
			"Hand this briefing to the deep-reasoner role (pi-subagents), then post the resulting judgement file back " +
	...
	lines.push(
		...
		"## 下一步",
		"",
		"在 pi 中运行 `/gatekeeper-triage`（或手工把本简报交给任意 agent 承担 deep-reasoner 角色），产出判断文件后运行：",
		"",
		"```",
